...
class ConvResidual(nn.Module):
    def __init__(self, dim, kernel_depthwise = 3, kernel_pointwise = 1):
        super().__init__()
        self.qkv_pos = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=kernel_depthwise, groups=dim, padding="same", bias = True),
            nn.BatchNorm2d(dim),
            nn.GELU(),
            nn.Conv2d(dim, dim, kernel_size=kernel_pointwise,padding=0, bias=False),
            nn.BatchNorm2d(dim),
            nn.GELU(),
        )
...
class ConvEmbedd(nn.Module):
    def __init__(self, dim, kernel_size = 3 ,
        ...
        self.conv_embed = torch.nn.Sequential(*(list(model.children())[:-2]))
        if input_size == 224:
            w = 7 #size feature map
        elif input_size == 32:
            w = 1 #size feature map
        self.proj = nn.Parameter(torch.zeros(patch_embed,w**2))
        trunc_normal_(self.proj, std=.02)
    def forward(self,x):
            x = self.conv_embed(x).flatten(2).transpose(2, 1)
            return self.proj @ x
...
class BlockConv(nn.Module):
    def __init__(self, dim, drop=0., drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, h=14, w=8):
        super().__init__()
        #self.norm1 = norm_layer(dim)
        self.filter = ConvResidual(dim)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        #self.norm2 = norm_layer(dim)
    def forward(self, x):
        x = x + self.drop_path((self.filter(x)))
        return x
...
class WayNet2(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12, depth_conv=4,
       ...
       #     for i in range(depth_conv)])
        ...
        #normalize cls token and position emb - it's important!!!!
